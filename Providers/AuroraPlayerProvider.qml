...
Singleton {
    ...
    // Treat two MPRIS entries as the same track only when their
    // metadata agrees. Timeline proximity is used only when titles
    // are unavailable, and its tolerances live in AuroraConfig.
    function isSameTrack(a, b) {
        const titleA = provider.normalized(a?.trackTitle)
        const titleB = provider.normalized(b?.trackTitle)
        const artistA = provider.normalized(a?.trackArtist)
        const artistB = provider.normalized(b?.trackArtist)
        const albumA = provider.normalized(a?.trackAlbum)
        const albumB = provider.normalized(b?.trackAlbum)
        ...
        const closePosition =
            ...
            AuroraConfig.duplicatePositionTolerance

        const closeLength =
            Math.abs((a?.length ?? 0) - (b?.length ?? 0)) <=
            AuroraConfig.duplicateLengthTolerance
