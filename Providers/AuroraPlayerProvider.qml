 ...
 * those before anything reaches AuroraState.players. Selecting a
 * source with selectPlayer() pins syncPlayer() to it until that
 * source disappears, at which point this falls back to
 * MprisController.activePlayer on its own.
 */
...
Singleton {
    ...
    // "" means automatic (follow MprisController.activePlayer, the
    // only mode that existed before this). Sits here, not in
    // AuroraState, since components never need the raw identity -
    ...
    // The pinned selection wins if it's still around; otherwise this
    // clears the pin and falls back to automatic instead of freezing
    // AuroraState on a source that just closed.
    ...
    function syncPlayerList() {
        ...
        AuroraState.players = provider.meaningfulPlayers.map(p => ({
            identity: p?.identity ?? "",
            title: p?.trackTitle ?? "",
            selected: p === active
        }))
    ...
    // tapping a chip in AuroraPlayerSwitcher feels instant.
    ...
    function syncPlayer() {
        ...
        // MprisController.activePlayer - same as before multi-player
        // selection existed, so a single-player setup behaves
