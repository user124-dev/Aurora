...
Singleton {
    ...
    function isSameTrack(a, b) {
        ...
        const closePosition = Math.abs((a.position ?? 0) - (b.position ?? 0)) <= AuroraConfig.duplicatePositionTolerance
        const closeLength = Math.abs((a.length ?? 0) - (b.length ?? 0)) <= AuroraConfig.duplicateLengthTolerance
