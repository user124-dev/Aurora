...
QtObject {
    ...
    // Floors so a bar never fully vanishes at extreme widths/levels.
    readonly property real spectrumMinBarWidth: 1
    readonly property real spectrumMinBarHeight: 2

    // Idle wave (no cava, but something is playing): level oscillates
    // between spectrumIdleBaseLevel and spectrumIdleBaseLevel +
    // spectrumIdleAmplitude, at spectrumIdleWaveSpeed radians per tick.
    // spectrumIdleRestLevel is the flat level when nothing plays at all.
    ...
    // Dimensions of the hand-drawn Previous/Next glyph (stop-bar +
    // triangle) - triangle width/height double as the ShapePath's own
    // coordinate bounds, so the vertex sits at exactly half height.
    ...
    // Play triangle and the paused-state two-bar glyph share the same
    // height on purpose - they occupy the same spot in the same
    // button as playback toggles, so they read as the same size.
    ...
    // Position offset for the small "1" repeat-one badge - negative,
    // so it sits slightly outside the repeat button's own corner.
