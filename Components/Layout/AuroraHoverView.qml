 ...
 * gives Aurora a fixed slot (hostSized). Spectrum is opt-in via
 * showSpectrum: hostSized widgets get it (there's no separate
 * Expanded gesture available to a host-embedded widget), the
 * floating hover popup doesn't - that's what tapping into
 * AuroraExpandedView is for instead.
 */
