...
`Layout/AuroraPlayer.qml` es el contenedor raíz. Controla el modo del widget (`Compact` / `Hover` / `Expanded`) según hover y tap, y delega el contenido a uno de tres archivos según el modo activo — `AuroraCompactView`, `AuroraHoverView` o `AuroraExpandedView` (ver `DECISIONS.md` → "Fase 3"). Cada subcomponente visual (`AuroraCover`, `AuroraInfo`, `AuroraSpectrum`, `AuroraControls`) es independiente y solo depende de `AuroraState` y `AuroraConfig` — nunca entre sí directamente.
