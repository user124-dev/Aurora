...
| `players` | list\<object\> | Fuentes MPRIS deduplicadas (`{identity, title, selected}`), alimentada por `AuroraPlayerProvider` — ver `DECISIONS.md` → "Fase 4: multi-player MPRIS" |
...
| Función | Descripción |
|---|---|
| `initialize()` | Sincroniza el estado inicial y la lista de players. Llamar una vez en `Component.onCompleted`. |
| `syncPlayer()` | Relee el player que se está mostrando (`resolveActivePlayer()`) y actualiza `AuroraState`. |
| `syncPlayerList()` | Reconstruye `AuroraState.players` a partir de `meaningfulPlayers`. Se llama solo cuando cambia el conjunto de players o la selección, no en cada tick de posición. |
...
| `selectPlayer(identity)` | Fija manualmente cuál fuente MPRIS mostrar y controlar. Si esa fuente desaparece, vuelve solo a modo automático — ver `DECISIONS.md` → "Fase 4: multi-player MPRIS". |

Escucha automáticamente `MprisController.onTrackChanged`, `onActivePlayerChanged` y cambios en `meaningfulPlayers` para re-sincronizar sin intervención del componente.
...
> Nota: este documento cubre la API interna actual (v0.1-dev). El punto todavía abierto es la reactividad de `AuroraThemeProvider` en modo `ThemeSystem` — ver `DECISIONS.md` → "Abierto / Pendiente".
