...
## cava config duplicado a propósito

`AuroraConfig.bars` (48) y `AuroraAudioProvider.maxRange` (1000) repiten
`bars = 48` y `ascii_max_range = 1000` de `Assets/cava/raw_output_config.txt`
- dos lugares con el mismo número. Evaluado leer el archivo directamente
en vez de mantener la duplicación; la conclusión es que no vale la pena:

- El archivo de cava es texto plano estilo INI, no JSON - `Quickshell.Io`
  no trae un parser para ese formato, habría que escribir uno a mano solo
  para extraer 2 números.
- Leerlo implica un `FileView` asíncrono. `AuroraConfig.bars` hoy es
  `readonly property int: 48`, disponible de inmediato para el binding de
  `AuroraSpectrum.qml` (`model: root.barCount`). Volverlo dependiente de
  una lectura de archivo lo convertiría en algo que se resuelve más tarde,
  justo lo que "Config vs. Theme" (arriba) descarta explícitamente para
  `AuroraConfig`: estático, no un sistema externo en vivo.
- Config file y AuroraConfig ya se documentan cruzados en comentarios
  (ver el propio `raw_output_config.txt`) - la duplicación no es silenciosa,
  es una decisión visible en ambos lados.

En vez de unificarlos, `AuroraAudioProvider` ahora valida en runtime: si
`cava` reporta una cantidad de barras distinta a `AuroraConfig.bars`, lo
loguea una vez (no en cada paquete, para no saturar el log al framerate=60
de cava) - no elimina la duplicación pero sí detecta si algún día se
desincroniza.
