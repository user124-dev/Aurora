...
One deliberate exception: `Components/Layout/AuroraPlayer.qml` (the root
widget) imports `Providers/` and calls `.initialize()` on each one in
`Component.onCompleted` - this is the single, controlled bootstrap point
that starts every Provider when the widget loads. It's not a leaf visual
component like the four above, and this rule was never meant to include
it; something has to call `initialize()`, and doing it anywhere else
would mean every leaf component needs its own bootstrap logic instead of
one shared root.
...
Today that isolation is real, not just architectural. `AuroraThemeProvider.qml`
has zero `qs.*` imports - it only reads `Themes/Default/Theme.qml`, Aurora's
own bundled palette, regardless of `AuroraConfig.themeMode`. This matches the
pivot documented in `CHANGELOG.md` ("Eliminadas las dependencias runtime
directas de `qs.services` y `qs.modules.common`"): Aurora no longer runs
embedded inside "ii", it's a standalone Quickshell configuration.

`usingSystemTheme` (`AuroraConfig.themeMode === AuroraConfig.themeSystem`) is
still computed but currently has no effect - `resolve()` always calls
`applyAuroraTheme()`. It's reserved for a future host theme adapter, not
dead code to remove; see `THEMES.md` → "Modos".
...
## Fase 5: Ecualizador — Nivel A (08/08/2026)

Primera pieza de `AuroraEqualizerProvider`, alcance deliberadamente
limitado a lo que `ROADMAP.md` v0.5 llama "Nivel A": cargar presets ya
guardados en EasyEffects, no editar bandas en vivo. La razón ya estaba
documentada antes de escribir código: EasyEffects no tiene una API de
D-Bus documentada para control de banda en vivo (ver "Spectrum data
source" arriba - mismo tipo de límite de protocolo, ahora aplicado a
EasyEffects en vez de MPRIS).

- **Detección antes que uso.** `AuroraEqualizerProvider` corre
  `command -v easyeffects` antes de tocar el filesystem o de escribir
  `AuroraState.equalizerAvailable = true`. Mismo orden que
  `AuroraAudioProvider` ya sigue para cava - EasyEffects es una
  dependencia opcional, su ausencia no debe romper nada más del widget.
- **Descubrimiento de presets vía filesystem, no CLI.** La CLI de
  EasyEffects no expone una bandera para *listar* presets, solo para
  *cargar* uno (`easyeffects -l <preset>`, ya confirmado en la fase
  anterior). Los presets viven como archivos `.json` sueltos en
  `~/.config/easyeffects/output/` (o `$XDG_CONFIG_HOME` equivalente) -
  el Provider lee ese directorio directamente con `find`, mismo
  mecanismo que `AuroraPluginRegistry` ya usa para descubrir
  `plugin.qml`.
- **Solo presets de salida (`output/`), no de entrada (`input/`).**
  EasyEffects separa presets de reproducción (salida) de presets de
  micrófono (entrada). Aurora es un widget de reproducción - los
  presets de entrada quedan fuera de alcance a propósito, no por
  descuido.
- **`AuroraState.equalizerAvailable` / `.equalizerPresets` /
  `.currentPreset`** siguen el mismo patrón que `players`: datos planos
  (nombres de string, nunca rutas de archivo) escritos únicamente por
  el Provider. `AuroraState.setPreset(name)` es la única acción pública,
  señal en vez de función reenviada - misma regla que el resto de
  `AuroraState` ya sigue (ver "Actions go through AuroraState").
- **Constante nueva en `AuroraConfig`:** `equalizerPresetScanDepth`
  (`1`) - profundidad de `find` al escanear presets. No es un límite de
  filesystem, es una decisión: un preset que otra herramienta anide en
  una subcarpeta no debe aparecer como si EasyEffects lo hubiera creado.

**Sin componente visual todavía.** Este cambio es solo el Provider y el
cableado en `AuroraState`/`AuroraConfig`. El panel de Ecualizador dentro
...
- **`themeSystem` aún no tiene adapter de host real.** En el runtime standalone,
  `AuroraConfig.themeMode === themeSystem` produce hoy el mismo resultado que
  `themeAurora` - `resolve()` siempre aplica la paleta bundled, sin leer ningún
  theme externo. Esto ya no es un bug de reactividad (no hay binding a un host
  que perder), es una feature pendiente: implementar un adapter real para un
  host específico es justo lo que el modo `themeSystem` está reservado a
  habilitar cuando exista una integración real que probar - ver `THEMES.md`.
