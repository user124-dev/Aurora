...
`Components/Media/AuroraBrowserBadge.qml` consume estos datos — un pill
junto a `AuroraInfo`, visible solo cuando `isBrowserPlaying` es `true`.
Lee únicamente `AuroraState.plugins.browserDetector`, nunca el plugin ni
el registro directamente — ver `DECISIONS.md` → "Fase 4: plugins de
terceros, el loop visual" y `Examples/Plugins/README.md` para los 2 casos
de prueba manual verificados end-to-end.
